...
import { RoomContext, LSRoomContext, SpecialEvent, ChatObject } from './room-context'
import { Room, Participant, LocalParticipant, RoomEvent, RemoteTrack, Track, AudioTrack, ParticipantEvent, ConnectionQuality, TrackPublication, LocalTrack } from 'livekit-client'
import { webrtcManager } from './index'
import { CoreContext } from '../context'

type RoomManagerState = LSRoomContext
const initialValue: RoomContext = null
const roomContextReducer: React.Reducer<RoomManagerState, LSRoomContext> = (prevState, action) => {
  return action
}

const participantsReducer = (prevState: Participant[], action: Participant[]) => {
  return action
}
...
const roughChatReducer: React.Reducer<ChatState, ChatState> = (prevState, action) => action

const chatReducer: React.Reducer<ChatState, ChatEvent> = (prevState, action) => {
  switch (action.type) {
    case 'POST':
...
export const useLivekitRoom = (roomName: string, token: string) => {
  ...
  const [roomContext, roomContextDispatch] =  useReducer(roomContextReducer, webrtcManager.ensureRoom(url.host + url.pathname, roomName, token))
  ...
  const [chatHistory, dispatchChat] = useReducer(roughChatReducer, roomContext.chatHistory)
  ...
  const [videoTracks, setVideoTracks] = useState(roomContext.participants.flatMap(p => p.getTracks().filter(t => t.kind === 'video')))
  const [audioTracks, setAudioTracks] = useState(roomContext.participants.flatMap(p => p.getTracks().filter(t => t.kind === 'audio')))
  ...
  const onParticipantsChanged = () => {
    ...
    setVideoTracks(roomContext.participants.flatMap(p => p.getTracks().filter(t => t.kind === 'video')))
    setAudioTracks(roomContext.participants.flatMap(p => p.getTracks().filter(t => t.kind === 'audio')))
  }
  ...
  useEffect(() => {
    roomContext.subscribeToRoomEvent(RoomEvent.ParticipantConnected, onParticipantsChanged)
    roomContext.subscribeToRoomEvent(RoomEvent.ParticipantDisconnected, onParticipantsChanged)
    roomContext.subscribeToRoomEvent(RoomEvent.ActiveSpeakersChanged, onParticipantsChanged)
    roomContext.subscribeToRoomEvent(RoomEvent.AudioPlaybackStatusChanged, onParticipantsChanged)
    roomContext.subscribeToRoomEvent(RoomEvent.Disconnected, onDisconnect)
    roomContext.subscribeToRoomEvent(RoomEvent.TrackSubscribed, onParticipantsChanged)
    roomContext.subscribeToRoomEvent(RoomEvent.TrackUnsubscribed, onParticipantsChanged)
    roomContext.subscribeToRoomEvent(RoomEvent.LocalTrackPublished, onParticipantsChanged)
    roomContext.subscribeToRoomEvent(RoomEvent.LocalTrackUnpublished, onParticipantsChanged)
    ...
    return () => {
      roomContext.unsubscribeFromRoomEvent(RoomEvent.ParticipantConnected, onParticipantsChanged)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.ActiveSpeakersChanged, onParticipantsChanged)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.ParticipantDisconnected, onParticipantsChanged)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.AudioPlaybackStatusChanged, onParticipantsChanged)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.Disconnected, onDisconnect)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.TrackSubscribed, onParticipantsChanged)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.TrackUnsubscribed, onParticipantsChanged)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.LocalTrackPublished, onParticipantsChanged)
      roomContext.unsubscribeFromRoomEvent(RoomEvent.LocalTrackUnpublished, onParticipantsChanged)
 ...
 */
export const useRoomEvent = (evt: RoomEvent, listener: (...args: any[]) => void) => {
  const { url, roomName } = useContext(LivekitContext)
  // TODO: use the actual SFU token in ensureRoom
  const [roomContext, roomContextDispatch] =  useReducer(roomContextReducer, webrtcManager.ensureRoom(url, roomName, CoreContext.state.activeProjectId))
  useEffect(() => {
...
export const useParticipant = (participant: Participant) => {
  ...
  const [connectionQuality, setConnectionQuality] = useState(participant?.connectionQuality ?? ConnectionQuality.Unknown)
  const onConnectionQualityChanged = (quality: ConnectionQuality) => {
    setConnectionQuality(quality)
  }

  const onPublishChanged = () => {
    const tracks = participant.getTracks().filter(t => t.track !== undefined)
    setPublications(tracks)
    setSubscribedTracks(tracks.filter(t => t.isSubscribed && t.track !== undefined))
  }
 ...
 */
export const LivekitContext = React.createContext<LivekitContext>({ url: '', roomName: '', token: ''})
